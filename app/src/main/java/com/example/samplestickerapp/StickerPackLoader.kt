...
object StickerPackLoader {
    ...
    private fun fetchFromContentProvider(cursor: Cursor): ArrayList<StickerPack> {
        ...
        do {
            ...
            val androidPlayStoreLink = cursor.getString(
            ...
            val publisherEmail = cursor.getString(
                cursor.getColumnIndexOrThrow(
                    StickerContentProvider.PUBLISHER_EMAIL
                )
            )
            val publisherWebsite = cursor.getString(
                cursor.getColumnIndexOrThrow(
                    StickerContentProvider.PUBLISHER_WEBSITE
                )
            )
            val privacyPolicyWebsite = cursor.getString(
                cursor.getColumnIndexOrThrow(
                    StickerContentProvider.PRIVACY_POLICY_WEBSITE
                )
            )
            val licenseAgreementWebsite = cursor.getString(
                cursor.getColumnIndexOrThrow(
                    StickerContentProvider.LICENSE_AGREENMENT_WEBSITE
                )
            )
            val imageDataVersion = cursor.getString(
                cursor.getColumnIndexOrThrow(
            ...
            val animatedStickerPack = cursor.getShort(
                cursor.getColumnIndexOrThrow(
                    StickerContentProvider.ANIMATED_STICKER_PACK
                )
            ) > 0
    ...
    private fun fetchFromContentProviderForStickers(
        ...
        if (cursor != null && cursor.count > 0) {
            ...
            do {
                ...
                val emojisConcatenated = cursor.getString(
                    cursor.getColumnIndexOrThrow(
                        StickerContentProvider.STICKER_FILE_EMOJI_IN_QUERY
                    )
                )
                ...
                if (!TextUtils.isEmpty(emojisConcatenated)) {
                    emojis = Arrays.asList(*emojisConcatenated.split(",".toRegex()).toTypedArray())
                }
